...
export default function BookCall() {
  ...
  return (
    <div className="bg-surface text-on-surface font-body selection:bg-primary/30 min-h-screen pb-32 relative">
      <Header />
      ...
      <main className="pt-24 px-6 max-w-4xl mx-auto">
        ...
        <div className="glass-panel p-6 rounded-2xl border border-outline-variant/15 mb-8 flex flex-col md:flex-row gap-6 items-center">
          <img
            alt="Doctor"
            className="w-24 h-24 rounded-full border border-primary/30 shadow-[0_0_20px_rgba(0,229,255,0.2)] object-cover"
          ...
          {slots.map((s, idx) => (
            <motion.div variants={itemVariants} key={idx}>
              <button
                ...
                className={`w-full py-4 rounded-xl border font-headline text-lg font-bold transition-all cursor-pointer ${selectedSlot === s ? 'bg-primary border-primary text-background shadow-[0_0_20px_rgba(0,229,255,0.4)] scale-105' : 'bg-surface-container-low border-outline-variant/15 text-on-surface hover:bg-surface-container-highest'}`}
              >
                {s}
              </button>
            </motion.div>
        ...
        <button
          ...
          className={`w-full liquid-gradient py-4 rounded-xl font-label text-xs uppercase tracking-[0.2em] font-bold text-on-primary shadow-lg transition-all ${!selectedSlot ? 'opacity-50 cursor-not-allowed filter grayscale' : 'cursor-pointer hover:shadow-[0_10px_30px_rgba(0,229,255,0.3)] active:scale-95'}`}
        >
