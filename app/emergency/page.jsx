...
export default function EmergencySOS() {
  ...
  const handleNavigate = async hospitalId => {
  ...
  const handleCallEMS = async hospitalId => {
    if (!userLocation) return;
    const targetHosp = hospitals.find(h => h.id === hospitalId);
    if (!targetHosp) return;
    setSelectedHospitalId(hospitalId);
    ...
    try {
      ...
      // Log dispatch to backend
      fetch("/api/emergency", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ hospitalId, lat: userLocation.lat, lon: userLocation.lon }),
  ...
  return <div className="bg-surface text-on-surface font-body selection:bg-primary/30 min-h-screen pb-32">
      <Header />
      ...
      <main className="pt-24 px-6 max-w-5xl mx-auto">
        ...
        stiffness: 200,
        damping: 20
      }} className="relative flex flex-col items-center justify-center py-12 mb-16">
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            <div className="w-96 h-96 bg-red-600/10 rounded-full blur-[100px]"></div>
          </div>
          <button onClick={playSiren} className="relative z-20 group cursor-pointer border-none outline-none">
            <div className={`w-64 h-64 md:w-80 md:h-80 rounded-full flex flex-col items-center justify-center transition-all duration-300 relative border-8 border-background ${isDispatching ? 'bg-red-600 shadow-[0_0_100px_rgba(255,0,0,0.8)] animate-pulse' : 'bg-red-600 shadow-[0_0_50px_rgba(255,0,0,0.4)] group-hover:scale-105 group-hover:bg-red-500 group-hover:shadow-[0_0_80px_rgba(255,0,0,0.6)]'}`}>
              <span className="font-headline text-6xl md:text-8xl font-black text-white tracking-tighter mb-2 drop-shadow-md">
                SOS
              </span>
              <span className="font-label text-xs md:text-sm text-white/90 tracking-[0.3em] font-bold uppercase transition-all drop-shadow-sm">
                {isDispatching ? "DISPATCHING..." : "Press to Dispatch"}
              </span>
              {/* Decorative scanner effect */}
              <div className={`absolute inset-0 rounded-full opacity-20 ${isDispatching ? 'border-4 border-white/80 border-dashed animate-[spin_3s_linear_infinite] opacity-100 scale-110' : 'border border-white/50 animate-ping duration-1000'}`}></div>
            </div>
          </button>
          ...
          repeat: Infinity,
          duration: 2
        }} className="mt-12 glass-panel px-8 py-3 rounded-full border border-red-500/30 flex items-center gap-3 bg-red-500/10">
            <div className="w-2 h-2 rounded-full bg-red-500 animate-pulse"></div>
            <span className="font-label text-xs uppercase tracking-widest text-red-600 font-bold drop-shadow-sm">
        ...
        <div className="space-y-8">
          ...
          <section className="glass-panel rounded-2xl border border-outline-variant/15 flex flex-col relative p-4 w-full gap-4">
            <div className="flex justify-between items-center z-20 shrink-0">
              <div>
                <h3 className="font-headline text-xl font-bold flex flex-wrap items-center gap-2">
                  ...
                  {!isOnline && <span className="font-label text-[10px] uppercase bg-error/20 text-error border border-error/50 px-2 py-1 rounded-full flex items-center gap-1 tracking-widest shadow-[0_0_10px_rgba(255,0,0,0.2)]">
              ...
              <button onClick={handleRecenter} className="bg-primary/10 p-2 rounded-lg hover:bg-primary/20 transition-colors cursor-pointer group flex items-center gap-2">
            ...
            <div className="w-full h-[400px] md:h-[500px] relative rounded-xl overflow-hidden glass-panel border border-outline-variant/20 shadow-inner z-10">
               <LeafletMap userLocation={userLocation} hospitals={hospitals} selectedHospitalId={selectedHospitalId} onHospitalSelect={setSelectedHospitalId} recenterTrigger={recenterTrigger} activeRoute={activeRoute || undefined} onNavigate={handleNavigate} isAmbulanceMode={dispatchedAmbulance !== null} />

               {/* Active Dispatch Radar Overlay */}
               {dispatchedAmbulance && <div className="absolute top-4 left-4 right-4 z-[2000] bg-[#1a0505]/95 backdrop-blur-xl border border-error/50 shadow-[0_10px_40px_rgba(255,0,0,0.3)] rounded-2xl p-4 flex items-center gap-4 transition-all animate-in fade-in slide-in-from-top-4 duration-500">
            ...
            <div className="lg:col-span-8 flex flex-col gap-6">
              <div className="glass-panel p-6 rounded-2xl border border-outline-variant/15 flex flex-col h-full min-h-[500px]">
                ...
                <div className="flex-1">
                  <HospitalList hospitals={hospitals} isLoading={isLoadingHospitals || isRouting} selectedHospitalId={selectedHospitalId} onSelect={setSelectedHospitalId} onNavigate={handleNavigate} onCallEMS={handleCallEMS} userLocation={userLocation} />
                </div>
              </div>
            </div>
