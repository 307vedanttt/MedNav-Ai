...
// ONCOLOGY
{
  ...
  distance: "14 Years",
  // Overloading this field for the primary card's "Experience"
  nextSlot: "2 Days",
  // Overloading this field for "Wait Time"
...
export default function NearbyDoctors() {
  ...
  const filteredDoctors = allDoctors.filter(doc => activeSpecialty === "All" || doc.specialty === activeSpecialty);
  return <div className="bg-surface text-on-surface font-body min-h-screen pb-32">
      <Header />
      ...
      <main className="pt-24 px-6 max-w-7xl mx-auto">
            ...
            <div className="flex flex-col md:flex-row gap-6">
              <div className="flex-1 space-y-2 relative z-10">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  return <button key={spec} onClick={() => setActiveSpecialty(spec)} className={`px-4 py-2 rounded-lg font-medium text-sm transition-transform cursor-pointer ${isActive ? 'bg-primary-container text-on-primary-fixed active:scale-95' : 'bg-surface-container-highest border border-outline-variant/15 text-on-surface-variant hover:text-primary transition-colors'}`}>
                        {spec}
                      </button>;
                })}
                </div>
              ...
              <div className="w-full md:w-64 space-y-2 relative z-10">
                ...
                <div className="flex items-center gap-4">
                  <input className="w-full accent-primary-container bg-surface-container h-1.5 rounded-full appearance-none cursor-pointer" type="range" min="1" max="50" value={proximity} onChange={e => setProximity(Number(e.target.value))} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-8 space-y-6">
            ...
            {filteredDoctors.length === 0 ? <div className="p-8 text-center bg-white rounded-xl border border-outline-variant/30">
                <p className="text-on-surface-variant">No specialists found in this category.</p>
              </div> : <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {filteredDoctors.map(doc => {
              if (doc.isPrimary) {
                return <div key={doc.id} className="glass-panel p-5 rounded-xl border border-outline-variant/10 hover:border-primary-container/30 transition-all hover:scale-[1.02] group md:col-span-2 block bg-white">
                        <div className="flex flex-col md:flex-row gap-6">
              ...
              return <div key={doc.id} className="glass-panel p-5 rounded-xl border border-outline-variant/10 hover:border-primary-container/30 transition-all hover:scale-[1.02] group block bg-white">
