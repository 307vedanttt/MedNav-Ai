...
export default function Login() {
  ...
  const handleSubmit = async e => {
    ...
    try {
      const res = await fetch("/api/auth", {
        ...
        body: JSON.stringify({ authId, passkey }),
      });
      ...
      localStorage.setItem("medai_user_id", data.userId);
  ...
  return <div className="bg-surface-dim font-body text-on-surface overflow-hidden min-h-screen flex items-center justify-center relative">
      ...
      <main className="relative z-10 w-full max-w-md px-6">
        ...
        <div className="glass-panel rounded-xl p-8 border border-outline-variant/15 neon-glow">
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            {error && (
              <div className="bg-error/10 text-error px-4 py-2 rounded-lg text-sm font-medium border border-error/20">
                {error}
              </div>
            )}
            <div>
              ...
              <div className="relative group">
                <span className="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 text-outline text-lg transition-colors group-focus-within:text-primary-container" data-icon="person">
                  person
                </span>
                <input className="w-full bg-surface-container-highest border-none rounded-lg py-4 pl-12 pr-4 text-on-surface placeholder:text-outline/40 focus:ring-1 focus:ring-primary-container/40 transition-all outline-none" placeholder="MD-7729-X" type="text" value={authId} onChange={e => setAuthId(e.target.value)} required />
              </div>
            </div>
            <div>
              <label className="block font-label text-[10px] uppercase tracking-[0.1em] text-on-surface-variant mb-2 ml-1">
              ...
              <div className="relative group">
                <span className="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 text-outline text-lg transition-colors group-focus-within:text-primary-container" data-icon="lock">
                  lock
                </span>
                <input className="w-full bg-surface-container-highest border-none rounded-lg py-4 pl-12 pr-12 text-on-surface placeholder:text-outline/40 focus:ring-1 focus:ring-primary-container/40 transition-all outline-none" placeholder="••••••••••••" type="password" value={passkey} onChange={e => setPasskey(e.target.value)} required />
                <button className="absolute right-4 top-1/2 -translate-y-1/2 text-outline hover:text-primary transition-colors cursor-pointer" type="button">
            ...
            <div className="flex items-center justify-between py-2">
              <label className="flex items-center space-x-2 cursor-pointer group">
                <input className="w-4 h-4 rounded bg-surface-container-highest border-outline-variant text-primary-container focus:ring-0 focus:ring-offset-0 cursor-pointer" type="checkbox" />
            ...
            <button className="w-full liquid-gradient py-4 rounded-xl font-headline font-bold text-on-secondary text-sm uppercase tracking-[0.1em] shadow-[0_4px_20px_rgba(0,229,255,0.2)] active:scale-95 transition-all duration-200 cursor-pointer disabled:opacity-50" type="submit" disabled={isLoading}>
