...
const itemVariants = { hidden: { opacity: 0, scale: 0.95, y: 20 }, show: { opacity: 1, scale: 1, y: 0, transition: { type: "spring" as const, stiffness: 300, damping: 24 } } };
...
export default function Dashboard() {
  ...
  return (
    <div className="bg-background selection:bg-primary-container selection:text-on-primary-container min-h-screen pb-32">
      {/* TopAppBar */}
      ...
      <main className="pt-28 px-6 max-w-7xl mx-auto">
        ...
        <motion.div variants={containerVariants} initial="hidden" animate="show" className="grid grid-cols-1 md:grid-cols-12 gap-6">
          ...
          {/* Bottom 4 items: Doctors, Book Call, Pharmacy, Vault (3 columns each) */}
          <motion.div variants={itemVariants} className="col-span-1 sm:col-span-6 md:col-span-3 h-full">
          ...
          <motion.div variants={itemVariants} className="col-span-1 sm:col-span-6 md:col-span-3 h-full">
            <Link href="/pharmacy" className="h-full bg-white rounded-2xl p-6 border border-outline-variant/30 hover:border-primary/40 shadow-sm hover:shadow-lg transition-all group flex flex-col items-center text-center">
              <div className="w-14 h-14 rounded-full bg-tertiary/10 text-tertiary flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
                <span className="material-symbols-outlined text-3xl" data-icon="medication">medication</span>
              </div>
              <h4 className="text-lg font-headline font-bold text-on-surface mb-2">Pharmacy</h4>
              <p className="text-sm font-light text-on-surface-variant">Manage and track prescriptions.</p>
            </Link>
          </motion.div>
