...
const specialtyConfig: Record<string, { icon: string; color: string; bg: string }> = {
  Cardiology: { icon: "cardiology", color: "text-error", bg: "bg-error/10" },
  Gastroenterology: { icon: "gastroenterology", color: "text-secondary", bg: "bg-secondary/10" },
  Neurology: { icon: "neurology", color: "text-primary", bg: "bg-primary/10" },
  Orthopedics: { icon: "skeleton", color: "text-tertiary-container", bg: "bg-tertiary-container/10" },
  General: { icon: "stethoscope", color: "text-on-surface-variant", bg: "bg-surface-container-highest" },
};
...
export default function HealthInsights() {
  ...
  const filteredHistory = filter === "All"
    ? history
    : history.filter(h => h.specialty === filter);
  ...
  const topSpecialty = history.length > 0
    ? Object.entries(
        history.reduce((acc, h) => { acc[h.specialty] = (acc[h.specialty] || 0) + 1; return acc; }, {} as Record<string, number>)
  ...
  return (
    <div className="bg-surface text-on-surface font-body selection:bg-primary-container selection:text-on-primary-container min-h-screen pb-32">
      <Header />
              ...
              <div className="space-y-2">
                ...
                <p className="text-on-surface-variant max-w-md text-sm">
                  {totalConsultations > 0 
                    ? `${totalConsultations} symptom${totalConsultations > 1 ? 's' : ''} analyzed. ${highSeverityCount > 0 ? `${highSeverityCount} flagged as high severity.` : 'No high severity flags detected.'}`
                    : 'No symptoms recorded yet. Use the Vitals tab to start your first consultation.'}
                </p>
              </div>
          ...
          <div className="lg:col-span-4 flex flex-col gap-6">
            <div className="flex-1 glass-panel rounded-xl p-6 border border-outline-variant/15">
              ...
              <div className="space-y-4">
                {totalConsultations === 0 ? (
                  <div className="p-4 bg-surface-container-high border-l-2 border-outline-variant rounded-r-lg">
                  ...
                  <>
                    {highSeverityCount > 0 && (
                      <div className="p-4 bg-error/5 border-l-2 border-error rounded-r-lg">
                    ...
                    <div className="p-4 bg-secondary/5 border-l-2 border-secondary rounded-r-lg">
                      <p className="text-sm text-on-surface leading-relaxed">
                        {totalConsultations >= 5 
                          ? "Regular health tracking detected. Your data profile is strengthening diagnostic accuracy."
                          : "Continue logging symptoms to improve AI prediction confidence."}
                      </p>
                    </div>
                  </>
                )}
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <button 
                onClick={() => alert("Comprehensive Medical Report generated successfully and securely saved to your local device.")}
                className="flex-[2] py-4 rounded-xl font-headline font-bold uppercase tracking-widest text-white bg-primary hover:bg-primary/90 shadow-[0_4px_15px_rgba(0,0,0,0.1)] transition-all cursor-pointer"
              >
                Generate Report
              </button>
              <button 
                onClick={clearHistory}
                className="flex-1 py-4 rounded-xl font-headline font-bold uppercase tracking-widest text-on-surface-variant border border-outline-variant/30 hover:bg-error/10 hover:text-error hover:border-error/30 transition-all cursor-pointer"
              >
                Clear History
              </button>
            </div>
        ...
        <section className="glass-panel rounded-xl p-6 border border-outline-variant/15">
            ...
            <div className="flex flex-wrap gap-2">
              {uniqueSpecialties.map((spec) => (
                <button 
                  ...
                  className={`px-3 py-1 rounded-full text-[10px] uppercase font-bold tracking-wider transition-all cursor-pointer border ${
                    filter === spec 
          ...
          {filteredHistory.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-on-surface-variant/50">
              ...
              <p className="text-xs">
                {history.length === 0 
                  ? 'Describe your symptoms on the Vitals page to begin tracking.' 
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <span className={`shrink-0 px-2 py-0.5 rounded text-[9px] uppercase font-bold tracking-wider ${
                            entry.severity === "High" 
