...
export default function SymptomsAnalysis() {
  ...
  const processSymptom = async text => {
    const newMsg = {
      id: Date.now(),
      role: "user",
      text: text
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        id: Date.now() + 1,
        role: "assistant",
        text: "Error connecting to Neural Symptom Engine. Please try again."
      }]);
    }
  };
  ...
  return <div className="bg-background text-on-surface font-body min-h-screen pb-40">
      <Header />
      ...
      <main className="pt-24 px-4 max-w-4xl mx-auto">
            ...
            <div className="flex flex-col md:flex-row gap-6 items-start">
              <div className="flex-1 w-full">
                ...
                <div className="space-y-4 mb-4">
                  ...
                  {showRecommendations && <motion.div initial={{
                  opacity: 0,
                  y: 20
                }} animate={{
                  opacity: 1,
        ...
        {!showRecommendations && <section className="mb-12">
          ...
          <div className="max-w-4xl mx-auto w-full pointer-events-auto">
            <form onSubmit={handleSubmit} className="glass-panel p-2 rounded-2xl border border-outline-variant/20 shadow-2xl neon-glow">
              <div className="flex items-center gap-2">
                <button type="button" onClick={startListening} className={`relative p-3 shrink-0 rounded-full transition-all cursor-pointer ${isListening ? 'bg-error/20 text-error shadow-[0_0_20px_rgba(255,0,0,0.4)]' : 'text-on-surface-variant hover:text-primary hover:bg-surface-variant'}`}>
                  <span className={`material-symbols-outlined ${isListening ? 'animate-pulse' : ''}`} data-icon="mic">
                    mic
                  </span>
                  {isListening && <span className="absolute inset-0 rounded-full border border-error animate-ping"></span>}
                </button>
                <input className="flex-1 min-w-0 bg-surface-container-highest/50 border-none rounded-xl text-sm py-3 px-4 focus:ring-1 focus:ring-primary-container placeholder:text-on-surface-variant/50 outline-none text-on-surface transition-all" placeholder="Type more details about your pain..." type="text" value={inputValue} onChange={e => setInputValue(e.target.value)} disabled={isTyping} />
