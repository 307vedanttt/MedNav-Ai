...
function MapUpdater({ center, zoom, trigger }: { center: [number, number]; zoom?: number, trigger?: number }) {
  const map = useMap();
  useEffect(() => {
...
const createCustomIcon = (isUser: boolean, isRecommended: boolean, isSelected: boolean) => {
...
export default function MapLayer({ userLocation, hospitals, selectedHospitalId, onHospitalSelect, recenterTrigger, activeRoute, onNavigate, isAmbulanceMode }: LeafletMapProps) {
  const defaultCenter = [40.7128, -74.0060]; // New York fallback if nothing is there
  const center: [number, number] = userLocation ? [userLocation.lat, userLocation.lon] : defaultCenter as [number, number];
  ...
  const [currentAmbulancePos, setCurrentAmbulancePos] = useState<[number, number] | null>(null);
  ...
  return (
    <div className="relative w-full h-full rounded-2xl overflow-hidden glass-panel border border-outline-variant/15 z-0">
      <MapContainer 
        center={center} 
        zoom={userLocation ? 14 : 12} 
        className="w-full h-full min-h-[400px] md:min-h-[600px] z-0" 
        zoomControl={false}
      >
        <TileLayer
          attribution='&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'
          url="https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png" // Dark theme OSM tiles
        />
        ...
        {activeRoute && activeRoute.length > 0 && (
          <>
            <Polyline 
              positions={isAmbulanceMode ? [...activeRoute].reverse() : activeRoute} 
              pathOptions={{ 
            ...
            />
            {isAmbulanceMode && currentAmbulancePos && (
              <Marker position={currentAmbulancePos} icon={ambulanceIcon} zIndexOffset={1000} />
            )}
          </>
        )}
        ...
        {userLocation && (
          <Marker 
            ...
            icon={createCustomIcon(true, false, false)}
          >
        ...
        {hospitals.map((hospital) => (
          <Marker 
            ...
            icon={createCustomIcon(false, !!hospital.recommended, selectedHospitalId === hospital.id)}
            eventHandlers={{
            ...
            <Popup className="custom-popup">
              <div className="flex flex-col gap-1 min-w-[200px]">
                {hospital.recommended && (
                  <span className="text-[10px] uppercase font-bold text-tertiary mb-1">Recommended</span>
                )}
                ...
                <button 
                  onClick={() => onNavigate ? onNavigate(hospital.id) : window.open(`https://www.openstreetmap.org/directions?engine=fossgis_osrm_car&route=${userLocation?.lat}%2C${userLocation?.lon}%3B${hospital.lat}%2C${hospital.lon}`, '_blank')}
                  className="mt-3 w-full py-1.5 rounded bg-primary text-white text-xs font-bold uppercase transition-colors hover:bg-primary/90"
                >
                  Navigate
                </button>
              </div>
